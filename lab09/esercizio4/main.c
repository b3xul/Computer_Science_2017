...
int main()
{
    ...
    do {
        ...
        switch (choice){
            ...
            case 3:
                printf("Name of the product to modify: ");
                scanf("%s",product_name);
                printf("Insert new price: ");
                scanf("%f",&new_price);

                res=update_product(warehouse,price,N,product_name,new_price);

                if (res==1)
                    printf("Update executed correctly.");
                else
            ...
            case 4:
                printf("Name of the product to remove: ");
                scanf("%s",product_name);
                res = remove_product(warehouse,price,N,product_name);

                if (res==0)
                    printf("Product not found.");
                else
                    printf("Product removed correctly.");
...
int insert_product(char warehouse[][M], float price[], int n, char new_product[], float price_new_product)
{
    ...
    if (found==0)
    {
        ...
        if (full==0)
        {   /* insertion of the new product and its price */
