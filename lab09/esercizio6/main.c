...
int main()
{
    ...
    printf("Note:\t- separate each element of a row with a space\n");
    printf("\t- at the end of a row press ENTER\n");
    printf("\t- after the last row (concluded by ENTER), press EOF\n");
    printf("\t- EOF is <CTRL> + Z in Windows, <CTRL> + D in Linux\n");
    while (flag && (scanf("%d%c", &value, &space) != EOF))
    {
        if (row == N)
            {
